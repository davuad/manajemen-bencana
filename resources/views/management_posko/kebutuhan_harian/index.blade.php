...
@section('content')
    <div class="bg-white rounded-xl shadow p-6">
        ...
        <div class="flex items-center gap-2 text-sm text-gray-500 mb-4">
            <a href="{{ route('management_posko.dapur_umum.index', ['role' => $userRole]) }}" class="hover:text-indigo-600">
                Management Posko
            </a>
            <span>/</span>
            <a href="{{ route('management_posko.dapur_umum.index', ['role' => $userRole]) }}" class="hover:text-indigo-600">
        ...
        <div class="flex justify-between items-center mb-6">
            ...
            {{-- TAMBAH DATA --}}
            <a href="{{ route('management_posko.kebutuhan_harian.create', ['role' => $userRole, 'dapur' => $dapur->id]) }}"
        ...
        {{-- FILTER --}}
        <form method="GET" action="{{ route('management_posko.kebutuhan_harian.index', ['role' => $userRole, 'dapur' => $dapur->id]) }}">
        ...
        <div class="flex justify-between items-center mt-6 text-sm">
            <p class="text-gray-500">
                Menampilkan {{ $kebutuhan->firstItem() ?? 0 }} - {{ $kebutuhan->lastItem() ?? 0 }} dari {{ $kebutuhan->total() }} data
            </p>
            <div>
            ...
            <div class="flex items-start gap-3">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-500 mt-1">
                        Apakah Anda yakin ingin menghapus data tanggal <span id="namaData" class="font-semibold"></span> ?
                    </p>
                </div>
            </div>
    ...
    <script>
        function openModal(id, nama) {
            ...
            {{-- PERBAIKAN DESTROY: Menyertakan parameter dapur --}}
            let url = "{{ route('management_posko.kebutuhan_harian.destroy', ['role' => $userRole, 'dapur' => $dapur->id, 'id' => ':id']) }}";
