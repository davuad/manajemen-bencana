...
<div class="mx-3 flex justify-between items-center">
    <div>
        ...
        <p class="text-gray-500 text-sm">Otorisasi final pengeluaran logistik dari gudang.</p>
    </div>
    <a href="{{ route('distribusi_bantuan.barang_keluar.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 px-4 py-2 rounded-lg text-sm transition font-medium">
        &larr; Kembali
    </a>
</div>

<form id="mainForm" action="{{ route('distribusi_bantuan.barang_keluar.update', $data->id) }}" method="POST">
    @csrf
    @method('PUT')
                ...
                <div class="space-y-4 text-sm">
                    <div class="bg-white/70 p-3 rounded-xl border border-red-100 shadow-sm">
                        ...
                        <div class="flex flex-col gap-2">
                            <a href="{{ route('distribusi_bantuan.pengajuan.show', $data->pengajuan_barang_id) }}" target="_blank" class="flex items-center gap-2 bg-indigo-600 text-white px-3 py-2 rounded-lg hover:bg-indigo-700 transition shadow-sm w-fit">
                        ...
                        @if(!in_array($data->status_proses, ['selesai', 'dibatalkan']))
                            <select name="petugas_gudang_id" id="select-petugas" class="w-full" required>
                                @foreach($pegawais as $p)
                                    <option value="{{ $p->id }}" {{ $data->petugas_gudang_id == $p->id ? 'selected' : '' }} data-jabatan="{{ $p->jabatan }}">
                                        {{ $p->nama_pegawai }}
                                    </option>
                                @endforeach
                            </select>
            ...
            <div class="bg-indigo-900 rounded-2xl p-5 shadow-lg text-white">
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <p class="text-[9px] text-indigo-300 uppercase font-bold leading-none mb-1">Waktu Update Terakhir</p>
                            <p class="text-xs font-medium">{{ $data->updated_at->translatedFormat('d F Y, H:i') }} WIB</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-[10px] text-red-500 font-medium italic">Selesaikan distribusi untuk memotong stok gudang secara otomatis.</p>
                </div>
            </div>
            ...
            <div class="flex gap-3 w-full md:w-auto">
                ...
                @endif
                <button type="submit" class="px-12 py-3 bg-indigo-600 text-white rounded-xl text-xs font-bold hover:bg-indigo-700 shadow-xl transition uppercase tracking-widest">Selesaikan Distribusi</button>
            </div>
        </div>
    </div>
    @endif
    ...
    <div id="statusModal" class="fixed inset-0 backdrop-blur-sm bg-black/40 hidden items-center justify-center z-50 p-4">
        <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md p-8 transform transition-all scale-100">
            ...
            <div class="grid grid-cols-2 gap-3 mt-8 border-t pt-6">
                ...
                <button type="submit" id="submitBtn" class="py-3 rounded-2xl text-white text-sm font-bold shadow-lg transition uppercase tracking-widest text-[10px]"></button>
            </div>
        </div>
    </div>
</form>
...
<script>
    ...
    function openStatusModal(status) {
        ...
        if (status === 'selesai') {
            ...
            submitBtn.className = 'py-3 rounded-2xl bg-green-600 hover:bg-green-700 text-white text-[10px] font-bold shadow-md uppercase tracking-widest';
        } else if (status === 'dikirim') {
            title.innerText = 'Kirim Barang?';
            desc.innerText = 'Status pengiriman berubah menjadi DIKIRIM (Dalam Perjalanan).';
            ...
            submitBtn.className = 'py-3 rounded-2xl bg-yellow-500 hover:bg-yellow-600 text-white text-[10px] font-bold shadow-md uppercase tracking-widest';
        } else {
            title.innerText = 'Batalkan Distribusi?';
            desc.innerText = 'Seluruh proses akan dihentikan tanpa ada pemotongan stok gudang.';
            ...
            submitBtn.className = 'py-3 rounded-2xl bg-red-600 hover:bg-red-700 text-white text-[10px] font-bold shadow-md uppercase tracking-widest';
        }
    }
