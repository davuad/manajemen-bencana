...
@endphp
    <div class="mx-3">
        <h2 class="text-xl font-bold">Edit Detail Paket Bantuan</h2>
        <p class="text-gray-500 text-sm">
            Perbarui data barang pada paket bantuan yang dipilih
        </p>
    </div>
    ...
    <div class="bg-white rounded-xl p-5 m-3 mt-5">
        ...
        <form action="{{ route($routePrefix .'.management_distribusi.detail_paket.update', $detail_paket->id) }}" method="POST"
            class="space-y-6">
            @csrf
            @method('PUT')

            <input type="hidden" name="paket_bantuan_id"
                value="{{ old('paket_bantuan_id', $detail_paket->paket_bantuan_id) }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" class="w-full border rounded-lg p-3 bg-gray-100"
                        value="{{ $detail_paket->paketBantuan->nama_paket ?? '-' }}" readonly>
                </div>

                <div>
                    <label class="block font-medium">Posko</label>
                    <input type="text" class="w-full border rounded-lg p-3 bg-gray-100"
                        value="{{ $detail_paket->paketBantuan->posko->nama_posko ?? '-' }}" readonly>
                </div>
                    ...
                    <select name="barang_id" id="barang_id" class="w-full border rounded-lg p-3" required>
                        ...
                        @foreach ($stok_barang as $stok)
                            <option value="{{ $stok->barang_id }}" data-satuan="{{ $stok->barang->satuan ?? '' }}"
                                {{ old('barang_id', $detail_paket->barang_id) == $stok->barang_id ? 'selected' : '' }}>
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="number" name="jumlah" class="w-full border rounded-lg p-3" min="1"
                        value="{{ old('jumlah', $detail_paket->jumlah) }}" placeholder="Masukkan jumlah barang" required>
                </div>

            </div>

            <div class="flex justify-end gap-3">
                <a href="{{ route($routePrefix .'.management_distribusi.detail_paket.index', ['paket_bantuan_id' => $detail_paket->paket_bantuan_id]) }}"
