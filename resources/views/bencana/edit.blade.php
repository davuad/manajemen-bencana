...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <div>
        ...
        <select name="kategori_id" class="w-full border rounded-lg p-3">
            @foreach($kategori as $k)
                <option value="{{ $k->id }}"
                    {{ $bencana->kategori_id == $k->id ? 'selected' : '' }}>
                    {{ $k->nama_kategori }}
                </option>
            @endforeach
        </select>
    ...
    <div>
        ...
        <select name="pengaduan_id" class="w-full border rounded-lg p-3">
            ...
            @foreach($pengaduan as $p)
                <option value="{{ $p->id }}"
                    {{ $bencana->pengaduan_id == $p->id ? 'selected' : '' }}>
                    {{ $p->deskripsi }}
                </option>
            @endforeach
        </select>
    ...
    <div>
        ...
        <select name="status_bencana" class="w-full border rounded-lg p-3">
            <option value="berlangsung"
                {{ $bencana->status_bencana == 'berlangsung' ? 'selected' : '' }}>
                🟠 Berlangsung
            </option>

            <option value="selesai"
                {{ $bencana->status_bencana == 'selesai' ? 'selected' : '' }}>
                🟢 Selesai
            </option>
        </select>
    </div>
    ...
    <div>
        ...
        <select name="tingkat_kerusakan" class="w-full border rounded-lg p-3">
            <option value="ringan" {{ $bencana->tingkat_kerusakan == 'ringan' ? 'selected' : '' }}>Ringan</option>
            <option value="sedang" {{ $bencana->tingkat_kerusakan == 'sedang' ? 'selected' : '' }}>Sedang</option>
            <option value="parah" {{ $bencana->tingkat_kerusakan == 'parah' ? 'selected' : '' }}>Parah</option>
        </select>
    </div>
...
<div class="flex justify-end mt-6 gap-3">
    <a href="{{ route('bencana.index') }}" class="bg-gray-300 px-4 py-2 rounded">
        Batal
    </a>
