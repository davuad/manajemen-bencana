...
<div class="bg-white rounded-xl p-5 m-3 mt-5">
    <form action="{{ route('management_korban.korban.update', $korban->id) }}"
          method="POST"
          class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <select name="bencana_id" class="w-full border rounded-lg p-3">
                    ...
                    @foreach ($bencana as $item)
                        <option value="{{ $item->id }}"
                            ...
                            {{ $item->kategori->nama_kategori ?? '-' }} - {{ $item->tanggal }}
                        </option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <select name="jenis_kelamin" class="w-full border rounded-lg p-3">
                    ...
                    <option value="Laki-Laki" {{ old('jenis_kelamin', $korban->jenis_kelamin) == 'Laki-Laki' ? 'selected' : '' }}>
                        Laki-Laki
                    </option>
                    <option value="Perempuan" {{ old('jenis_kelamin', $korban->jenis_kelamin) == 'Perempuan' ? 'selected' : '' }}>
                        Perempuan
                    </option>
                </select>
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <input type="text"
                    name="alamat"
                    value="{{ old('alamat', $korban->alamat) }}"
                    class="w-full border rounded-lg p-3"
                    placeholder="Masukkan alamat korban">
            </div>
            ...
            <div>
                ...
                <input type="text"
                    ...
                    value="{{ old('lokasi_kejadian', $korban->lokasi_kejadian) }}"
                    class="w-full border rounded-lg p-3"
