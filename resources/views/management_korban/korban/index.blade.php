...
<div class="bg-white rounded-xl shadow p-6">
    ...
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('management_korban.korban.create') }}"
    ...
    <!-- Success Message -->
    @if(session('success'))
        <div id="alert-success" class="mb-4 rounded-lg bg-green-100 px-4 py-3 text-green-700 transition-opacity duration-500">
            {{ session('success') }}
        </div>
    ...
    <form method="GET" action="{{ route('management_korban.korban.index') }}">
            ...
            <select name="bencana_id" class="border rounded-lg py-2 px-3">
                ...
                @foreach ($bencana as $item)
                    <option value="{{ $item->id }}"
                        ...
                        {{ $item->kategori->nama_kategori ?? '-' }} - {{ $item->desa->nama_desa ?? '-' }} - {{ $item->tanggal }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="flex flex-wrap gap-3 mb-6">
            <a href="{{ route('management_korban.korban.reviewPdf', request()->query()) }}"
               target="_blank"
                    ...
                    <tr class="border-t">
                        ...
                        <td class="py-3">
                            <div class="flex gap-2 items-center">
                                <a href="{{ route('management_korban.korban.show', $item->id) }}"
                                   class="text-green-600 hover:text-green-800"
                                   title="Detail">
                                    <x-heroicon-o-eye class="w-5 h-5" />
                                </a>

                                <a href="{{ route('management_korban.korban.edit', $item->id) }}"
...
function openModal(id, nama) {
    ...
    let url = "{{ route('management_korban.korban.destroy', ':id') }}";
