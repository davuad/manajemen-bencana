...
<div class="bg-white rounded-xl p-5 m-3 mt-5">
    ...
    <form action="{{ route('admin.management_pegawai.relawan.update', $relawan->id_relawan) }}"
          method="POST"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="jenis_psks" class="w-full border rounded-lg p-3">
                    ...
                    <option value="pordam"
                        {{ old('jenis_psks', $relawan->jenis_psks) == 'pordam' ? 'selected' : '' }}>
                        pordam
                    </option>

                    <option value="karang taruna"
                        {{ old('jenis_psks', $relawan->jenis_psks) == 'karang taruna' ? 'selected' : '' }}>
                        karang taruna
                    </option>

                    <option value="tagana"
                        {{ old('jenis_psks', $relawan->jenis_psks) == 'tagana' ? 'selected' : '' }}>
                        tagana
                    </option>

                    <option value="tsks"
                        {{ old('jenis_psks', $relawan->jenis_psks) == 'tsks' ? 'selected' : '' }}>
                        tsks
                    </option>
                </select>
            ...
            <div class="md:col-span-2">
                <label class="block font-medium mb-2">Foto Relawan</label>
                ...
                @if($relawan->foto)
                    <img src="{{ asset('storage/'.$relawan->foto) }}"
                         class="w-28 h-28 mt-3 rounded-lg object-cover">
                @endif
