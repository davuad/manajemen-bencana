...
@section('content')
    ...
    <div class="bg-white rounded-xl shadow p-6">
        ...
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="{{ route('admin.management_user.create') }}"
        ...
        <div class="flex gap-4 mb-6">
            ...
            <form action="{{ route('admin.management_user.index') }}" method="GET" class="flex gap-4 mb-6">
                ...
                @if (request('search') || request('role') || request('status'))
                    <a href="{{ route('admin.management_user.index') }}"
                        class="px-4 py-2 text-sm text-gray-600 hover:text-gray-900 underline flex items-center justify-center">
                        Reset
                    </a>
                @endif
                ...
                <tbody>
                    @forelse($users as $key => $user)
                        <tr class="border-t">
                            ...
                            <td>
                                @if ($user->foto)
                                    <img src="{{ asset('storage/' . $user->foto) }}" alt="Foto {{ $user->nama }}"
                            ...
                            <td class="p-2">
                                @forelse($user->roles as $role)
                                    <span
                                        class="inline-block px-2 py-1 rounded bg-blue-200 text-blue-800 text-xs font-semibold">
                                        {{ ucfirst($role->name) }}
                                    </span>
                                @empty
                            ...
                            <td class="p-2 text-center">
                                @if ($user->status == 'aktif')
                                    <span
                                        class="inline-block px-4 py-2 rounded-full bg-green-200 text-green-800 font-semibold">
                                        Aktif
                                    </span>
                                @else
                                    <span
                                        class="inline-block px-4 py-2 rounded-full bg-red-200 text-red-700 font-semibold opacity-70">
                                        Nonaktif
                                    </span>
                                @endif
                            </td>
                            ...
                            <td class="flex gap-1 py-4">
                                <button onclick="openDetailModal(this)" data-user='@json($user)'
                                ...
                                @if ($user->hasRole('admin'))
                                    <button disabled class="text-gray-300 cursor-not-allowed"
                                        title="Tidak dapat mengedit admin">
                                        <x-heroicon-o-pencil-square class="w-5 h-5" />
                                    </button>
                                @else
                                    <a href="{{ route('admin.management_user.edit', $user->id) }}"
                                ...
                                @else
                                    <button onclick="openModal('{{ $user->id }}', '{{ $user->nama }}')"
    ...
    <script>
        function openModal(id, nama) {
            ...
            // set dynamic route
            document.getElementById('deleteForm').action = `/admin/management-user/${id}`;
        }
