    ...
    <div class="space-y-6">
        ...
        <div class="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
            ...
            <div class="px-6 py-6 md:px-8 md:py-8">
                <form action="{{ route('admin.desa.update', $desa->id) }}" method="POST" class="space-y-8">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2 md:gap-x-10 md:gap-y-7">
                        ...
                        <div>
                            ...
                            <input type="text" id="nama_desa" name="nama_desa"
                                value="{{ old('nama_desa', $desa->nama_desa) }}" placeholder="Masukkan nama desa"
                        ...
                        <div>
                            ...
                            <input type="text" id="nama_kades" name="nama_kades"
                                value="{{ old('nama_kades', $desa->nama_kades) }}" placeholder="Masukkan nama kepala desa"
                                class="w-full rounded-xl border border-gray-300 bg-white px-4 py-3 text-sm text-gray-900 outline-none transition focus:border-indigo-600 focus:ring-4 focus:ring-indigo-100">
                            @error('nama_kades')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="text" id="kecamatan" name="kecamatan"
                                value="{{ old('kecamatan', $desa->kecamatan) }}" placeholder="Masukkan kecamatan"
                                class="w-full rounded-xl border border-gray-300 bg-white px-4 py-3 text-sm text-gray-900 outline-none transition focus:border-indigo-600 focus:ring-4 focus:ring-indigo-100">
                            @error('kecamatan')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="text" id="kontak_kades" name="kontak_kades"
                                value="{{ old('kontak_kades', $desa->kontak_kades) }}"
