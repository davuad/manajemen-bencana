...
@endphp
    <div class="space-y-6">
        <div class="text-sm text-gray-500">
            Dashboard <span class="mx-1">&gt;</span> Data Warga Terdampak <span class="mx-1">&gt;</span> Edit Data Warga Terdampak
        </div>

        <div class="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
            <div class="border-b border-gray-100 px-6 py-5">
                <h2 class="text-xl font-bold text-gray-900">Edit Data Warga Terdampak</h2>
            </div>

            <div class="px-6 py-6 md:px-8 md:py-8">
                <form action="{{ route($userRole . '.warga.update', $warga->id) }}" method="POST" class="space-y-8">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2 md:gap-x-8 md:gap-y-7">
                        ...
                        <div>
                            ...
                            <select id="jenis_bantuan" name="jenis_bantuan"
                                ...
                                <option value="Bantuan Saat Bencana" {{ old('jenis_bantuan', $warga->jenis_bantuan) == 'Bantuan Saat Bencana' ? 'selected' : '' }}>Bantuan Saat Bencana</option>
                                <option value="Bantuan Pasca Bencana" {{ old('jenis_bantuan', $warga->jenis_bantuan) == 'Bantuan Pasca Bencana' ? 'selected' : '' }}>Bantuan Pasca Bencana</option>
                            </select>
                        ...
                        <div>
                            ...
                            <select id="status_penyaluran" name="status_penyaluran"
                                ...
                                <option value="Belum diproses" {{ old('status_penyaluran', $warga->status_penyaluran) == 'Belum diproses' ? 'selected' : '' }}>Belum Diproses</option>
                                <option value="Proses Penyaluran" {{ old('status_penyaluran', $warga->status_penyaluran) == 'Proses Penyaluran' ? 'selected' : '' }}>Proses Penyaluran</option>
                                <option value="Sudah disalurkan" {{ old('status_penyaluran', $warga->status_penyaluran) == 'Sudah disalurkan' ? 'selected' : '' }}>Sudah Disalurkan</option>
                            </select>
                        ...
                        <div>
                            ...
                            <select id="desa_id" name="desa_id"
                                ...
                                @foreach ($desa as $item)
                                    <option value="{{ $item->id }}" {{ old('desa_id', $warga->desa_id) == $item->id ? 'selected' : '' }}>{{ $item->nama_desa }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="bencana_id" name="bencana_id"
                                ...
                                @foreach ($bencana as $item)
                                    <option value="{{ $item->id }}" {{ old('bencana_id', $warga->bencana_id) == $item->id ? 'selected' : '' }}>{{ $item->nama_bencana }}</option>
                                @endforeach
                            </select>
