...
<div class="bg-white rounded-xl shadow p-6 m-3 mt-5">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        ...
        <div class="flex items-center gap-3">
            {{-- TOMBOL CETAK PDF (Bisa diakses Admin & Kabid) --}}
            @if($isAdminPage || $isKabidPage)
            <a href="{{ route($prefix . '.jadwal.cetak', request()->all()) }}" target="_blank" class="flex items-center gap-2 bg-white border border-blue-200 text-blue-600 hover:bg-red-50 px-4 py-2 rounded-lg transition font-black text-[10px] shadow-sm uppercase tracking-widest">
                Cetak PDF
            </a>
            @endif

            {{-- TOMBOL TAMBAH JADWAL (Hanya Admin) --}}
            @if($isAdminPage)
            <a href="{{ route('admin.jadwal.create') }}" class="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg transition font-black text-[10px] shadow-lg uppercase tracking-widest">
                Tambah Jadwal
            </a>
            @endif
        </div>
    ...
    <form method="GET" action="{{ route($prefix . '.jadwal.index') }}" class="flex flex-wrap items-center gap-2 mb-6 p-4 bg-gray-50 rounded-lg border border-gray-100">
        ...
        <input type="number" name="tahun_mulai"
            value="{{ request('tahun_mulai', $tahunMulai) }}"
            placeholder="Dari Tahun"
            class="text-[11px] p-2 border border-gray-200 rounded-lg bg-white w-28">

        <span class="text-xs text-gray-400 font-bold">s/d</span>

        <input type="number" name="tahun_selesai"
            value="{{ request('tahun_selesai', $tahunSelesai) }}"
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50 transition text-gray-700">
                    ...
                    <td class="p-4 font-medium">
                        ...
                        {{ $jadwal->bencana ? \Carbon\Carbon::parse($jadwal->bencana->tanggal)->locale('id')->translatedFormat('d F Y') : '-' }}
                    </td>
                    ...
                    {{-- PERBAIKAN LOGIKA STATUS (Anti-Error) --}}
                    <td class="p-4 text-center">
                        @php
                        // Membersihkan data dari spasi gaib & huruf besar
                        $status = strtolower(trim($jadwal->status));
                        @endphp
                        <span class="px-2 py-1 rounded-md font-bold text-[9px] uppercase border {{ $status === 'selesai' ? 'bg-emerald-100 text-emerald-800 border-emerald-200' : 'bg-blue-100 text-blue-800 border-blue-200' }}">
                            {{ $jadwal->status }}
                        </span>
                    </td>
                ...
                @empty
                <tr>
                    <td colspan="11" class="text-center p-12 text-gray-400 italic">Belum ada data jadwal.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
...
{{-- MODAL HAPUS (Ditampilkan hanya untuk Admin agar tidak bocor ke role lain) --}}
@if($isAdminPage)
