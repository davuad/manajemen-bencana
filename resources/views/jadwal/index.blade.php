    ...
    <div class="bg-white rounded-xl shadow p-6 m-3 mt-5">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('admin.jadwal.cetak', request()->all()) }}" target="_blank"
        ...
        <form method="GET" action="{{ route('admin.jadwal.index') }}"
            class="flex flex-wrap items-center gap-2 mb-6 p-4 bg-gray-50 rounded-lg border border-gray-100">
            ...
            <button type="submit"
                class="bg-indigo-600 text-white px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-indigo-700 transition">
                Filter
            </button>
            <a href="{{ route('admin.jadwal.index') }}"
                class="text-[10px] text-gray-400 font-bold hover:text-gray-600 uppercase tracking-widest ml-2">
                Reset
            </a>
        </form>
            ...
            <table class="w-full text-xs text-left min-w-[1200px]">
                <thead class="bg-gray-50 text-gray-600 uppercase font-black border-b border-gray-200">
                    <tr>
                        ...
                        <th class="p-4 text-left min-w-[160px]">Nama Bencana</th>
                        <th class="p-4 text-left min-w-[160px]">Jenis Layanan</th>
                        <th class="p-4 text-left min-w-[160px]">Target</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($jadwals as $key => $jadwal)
                        {{-- Class 'uppercase' di tr ini sudah dihapus agar teks tidak kapital paksa --}}
                        <tr class="hover:bg-gray-50 transition">
                            <td class="p-4 text-center text-gray-700 font-bold">{{ $key + 1 }}</td>
                            <td class="p-4 text-left">
                                <div class="font-black text-gray-800 text-[13px] tracking-wide">
                            ...
                            <td class="p-4 text-left text-gray-700 font-medium">{{ $jadwal->pegawai->nama_pegawai ?? '-' }}
                            </td>
                            ...
                            <td class="p-4 text-center">
                                @if ($jadwal->status == 'selesai')
                                    <span
                                        class="px-2.5 py-1 bg-emerald-100 text-emerald-800 rounded-md font-bold text-[9px] tracking-widest uppercase border border-emerald-200">SELESAI</span>
                                @else
                                    <span
                                        class="px-2.5 py-1 bg-blue-100 text-blue-800 rounded-md font-bold text-[9px] tracking-widest uppercase border border-blue-200">DIJADWALKAN</span>
                                @endif
                            </td>
                            <td class="p-4 text-center">
                                <div class="flex gap-2 justify-center">
                                    <a href="{{ route('admin.jadwal.edit', $jadwal->id) }}"
                                        class="p-2 text-blue-500 hover:bg-blue-50 rounded-lg transition border border-transparent hover:border-blue-200">
                                        <x-heroicon-o-pencil-square class="w-4 h-4" />
                                    </a>

                                    {{-- Proses pembuatan Teks Gabungan Bencana - Desa - Tahun --}}
                                    @php
                                        $namaBencana = $jadwal->bencana->nama_bencana ?? 'Bencana';
                                        $namaDesa = $jadwal->bencana->desa->nama_desa ?? 'Desa';
                                        $tahunBencana = $jadwal->bencana
                                            ? \Carbon\Carbon::parse($jadwal->bencana->tanggal)->format('Y')
                                            : '-';

                                        // Gabungkan jadi satu string utuh
                                        $teksHapus = addslashes("$namaBencana - $namaDesa - $tahunBencana");
                                    @endphp

                                    <button type="button"
                                        onclick="openModal('{{ $jadwal->id }}', '{{ $teksHapus }}')"
                                        class="p-2 text-red-500 hover:bg-red-50 rounded-lg transition border border-transparent hover:border-red-200">
                    ...
                    @empty
                        <tr>
                            <td colspan="11" class="text-center p-12 text-gray-400 italic">Belum ada jadwal layanan yang
                                diinput.</td>
                        </tr>
                    @endforelse
                </tbody>
            ...
            <div class="flex items-start gap-3">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-500 mt-1">
                        ...
                        <span id="namaJadwal" class="font-bold text-red-600"></span> ?
                    </p>
                </div>
            </div>
