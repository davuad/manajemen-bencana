...
<form action="{{ route('stok_gudang.update', $stok->id) }}" method="POST" class="space-y-6">
@csrf
@method('PUT')
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <div>
        ...
        <select name="kondisi_barang" class="w-full border rounded-lg p-3">
            <option value="baik" {{ old('kondisi_barang', $stok->kondisi_barang) == 'baik' ? 'selected' : '' }}>Baik</option>
            <option value="rusak" {{ old('kondisi_barang', $stok->kondisi_barang) == 'rusak' ? 'selected' : '' }}>Rusak</option>
        </select>
    </div>
